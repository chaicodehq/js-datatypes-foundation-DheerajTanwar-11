...
export function isValidPincode(code) {
  ...
  if (typeof code !== 'string' || code.startsWith("0") || !isDigitRegex.test(code) || code.length !== 6) return false ; 
  return true ; 
}
...
export function formatPostcardField(label, value, width) {
  ...
  return label.padEnd(width !== undefined ? width : 12) + ': ' + value ; 
...
export function countVowels(message) {
  // Your code here
  if (typeof message !== 'string' || message.length === 0 || message === null) return 0;

  const vowelsRegex = /[aeiouAEIOU]/g ; 

  let vowels = message.match(vowelsRegex);
  
  let vowelsCounts = vowels !== null ? vowels.length : 0 ; 
