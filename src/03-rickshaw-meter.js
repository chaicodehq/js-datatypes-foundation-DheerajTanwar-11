...
export function parseFare(fareString) {
  ...
  if(Number.isNaN(parseFloat(fareString))) return -1; 
...
export function findCheapestAndCostliest(...fares) {
  // Your code here
  let validfaresnumber = fares.filter((e) => typeof e === 'number' && !isNaN(e)) ; 
  if (validfaresnumber.length === 0) return null ; 

  return {
    cheapest: Math.min(...validfaresnumber),
    costliest: Math.max(...validfaresnumber)
  }
}
